...
{
	$queryAuthorizeStaff = "SELECT * FROM as_modules WHERE modulID = '31'";
	...
	if ($module == 'paykhs' && $act == 'delete')
	{
		...
		if ($q != '')
		{
			header("Location: pay_khs.php?module=paykhs&act=search&q=".$q."&msg=Data transaksi penjualan berhasil dihapus");
		}
		else
		{
			header("Location: pay_khs.php?msg=Data transaksi penjualan berhasil dihapus");
		}
	} // close bracket
	
	// if the module is pay out and act is input
	elseif ($module == 'paykhs' && $act == 'input')
	{
		...
		header("Location: pay_khs.php?module=paykhs&act=finish&paymentNo=".$paymentNo."&paymentID=".$paymentID."&invoiceNo=".$invoiceNo);
	}
	
	// if the module is pay out and act is detail pay out
	elseif ($module == 'paykhs' && $act == 'detailpayout')
	{
		$paymentID = $_GET['paymentID'];
		$paymentNo = $_GET['paymentNo'];
		$invoiceNo = $_GET['invoiceNo'];
		...
		$smarty->assign("breadcumbTitle", "Pembayaran Transaksi Penjualan Khusus");
		$smarty->assign("breadcumbTitleSmall", "Halaman untuk melakukan pembayaran transaksi penjualan khusus.");
		$smarty->assign("breadcumbMenuName", "Transaksi Penjualan Khusus");
		$smarty->assign("breadcumbMenuSubName", "Pembayaran Transaksi Khusus");
	}
	
	// if module out and act is finish
	elseif ($module == 'paykhs' && $act == 'finish')
	{
		$paymentNo = $_GET['paymentNo'];
		$paymentID = $_GET['paymentID'];
		$invoiceNo = $_GET['invoiceNo'];
	...
	elseif ($module == 'paykhs' && $act == 'add')
	{
		...
		$smarty->assign("breadcumbTitle", "Transaksi Penjualan Khusus");
		$smarty->assign("breadcumbTitleSmall", "Halaman untuk melakukan transaksi penjualan, faktur penjualan.");
		$smarty->assign("breadcumbMenuName", "Transaksi Penjualan Khusus");
		$smarty->assign("breadcumbMenuSubName", "Transaksi Penjualan Khusus");
	}

	elseif ($module == 'paykhs' && $act == 'search')
	{
		$q = mysqli_real_escape_string($connect, $_GET['q']);
